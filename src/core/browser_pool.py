# src/core/browser_pool.py
"""
StealthMaster AI v3.0 - Enhanced Browser Pool
High-performance browser pooling with CDP bypass integration
"""

import asyncio
import logging
import time
from typing import Dict, List, Optional, Tuple, Any, AsyncContextManager
from dataclasses import dataclass, field
from datetime import datetime, timedelta
from contextlib import asynccontextmanager
from playwright.async_api import Browser, BrowserContext, Page, async_playwright, Playwright
import psutil

# Import v3 stealth components
from ..stealth.cdp_bypass_engine import get_cdp_bypass_engine
from ..stealth.stealth_integration import StealthIntegration
from ..stealth.advanced_fingerprint import AdvancedFingerprint

logger = logging.getLogger(__name__)


@dataclass
class BrowserInstance:
    """Enhanced browser instance with stealth features"""
    browser: Browser
    playwright: Playwright
    created_at: datetime
    last_used: datetime
    usage_count: int = 0
    health_check_failures: int = 0
    is_healthy: bool = True
    contexts: List[BrowserContext] = field(default_factory=list)
    fingerprint: Dict[str, Any] = field(default_factory=dict)
    cdp_protected: bool = False
    
    @property
    def age_seconds(self) -> float:
        """Get age of browser instance in seconds"""
        return (datetime.now() - self.created_at).total_seconds()
    
    @property
    def idle_seconds(self) -> float:
        """Get idle time in seconds"""
        return (datetime.now() - self.last_used).total_seconds()
    
    @property
    def requests_handled(self) -> int:
        """Get total requests handled by this instance"""
        return self.usage_count


class EnhancedBrowserPool:
    """
    Enhanced browser pool with v3.0 features:
    - CDP bypass integration
    - Fingerprint rotation
    - Stealth measure application
    - Intelligent recycling
    """
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        self.config = config or {}
        
        # Pool settings optimized for stealth
        self.min_size = self.config.get('min_size', 2)
        self.max_size = self.config.get('max_size', 5)
        self.max_age_seconds = self.config.get('max_age_seconds', 600)  # 10 minutes
        self.max_idle_seconds = self.config.get('max_idle_seconds', 180)  # 3 minutes
        self.max_requests_per_browser = self.config.get('max_requests_per_browser', 20)
        self.health_check_interval = self.config.get('health_check_interval', 60)
        
        # Browser options
        self.browser_options = {
            'headless': self.config.get('headless', False),  # Never headless for stealth
            'channel': self.config.get('channel', 'chrome'),
            'args': self._get_stealth_args()
        }
        
        # Pool state
        self.pool: List[BrowserInstance] = []
        self.in_use: List[BrowserInstance] = []
        self.lock = asyncio.Lock()
        self.initialized = False
        self.shutdown = False
        
        # CDP bypass engine
        self.cdp_bypass = get_cdp_bypass_engine()
        
        # Statistics
        self.stats = {
            'browsers_created': 0,
            'browsers_recycled': 0,
            'pool_hits': 0,
            'pool_misses': 0,
            'health_checks': 0,
            'cdp_protections_applied': 0,
            'fingerprints_rotated': 0,
            'avg_acquisition_time': 0.0,
            'total_acquisitions': 0
        }
        
        # Background tasks
        self._health_check_task: Optional[asyncio.Task] = None
        self._maintenance_task: Optional[asyncio.Task] = None
        
    def _get_stealth_args(self) -> List[str]:
        """Get browser arguments for stealth"""
        return [
            '--no-sandbox',
            '--disable-setuid-sandbox',
            '--disable-blink-features=AutomationControlled',
            '--disable-dev-shm-usage',
            '--disable-web-security',
            '--disable-features=IsolateOrigins,site-per-process',
            '--disable-site-isolation-trials',
            '--disable-background-timer-throttling',
            '--disable-backgrounding-occluded-windows',
            '--disable-renderer-backgrounding',
            '--disable-features=TranslateUI',
            '--disable-features=UserAgentClientHint',
            '--window-size=1920,1080',
            '--start-maximized'
        ]
    
    async def initialize(self) -> None:
        """Initialize the enhanced browser pool"""
        if self.initialized:
            return
        
        logger.info(f"🚀 Initializing Enhanced Browser Pool (min={self.min_size}, max={self.max_size})")
        
        # Pre-warm the pool
        await self._ensure_minimum_pool_size()
        
        # Start background tasks
        self._health_check_task = asyncio.create_task(self._health_check_loop())
        self._maintenance_task = asyncio.create_task(self._maintenance_loop())
        
        self.initialized = True
        logger.info(f"✅ Browser pool initialized with {len(self.pool)} stealth browsers")
    
    async def _create_browser_instance(self) -> BrowserInstance:
        """Create a new browser instance with full stealth measures"""
        
        logger.debug("Creating new stealth browser instance...")
        
        # Start Playwright
        playwright = await async_playwright().start()
        
        # Generate unique fingerprint
        fingerprint = AdvancedFingerprint.generate_fingerprint()
        
        # Launch with stealth measures
        browser = await StealthIntegration.create_stealth_browser(playwright, self.browser_options['headless'])
        
        # Create instance
        instance = BrowserInstance(
            browser=browser,
            playwright=playwright,
            created_at=datetime.now(),
            last_used=datetime.now(),
            fingerprint=fingerprint
        )
        
        self.stats['browsers_created'] += 1
        logger.info(f"✅ Created stealth browser #{self.stats['browsers_created']}")
        
        return instance
    
    async def _get_browser_instance(self) -> BrowserInstance:
        """Get a browser instance from pool or create new"""
        
        async with self.lock:
            # Try to get from pool
            while self.pool:
                instance = self.pool.pop(0)
                
                # Check if instance is still healthy and not too old
                if (instance.is_healthy and 
                    instance.age_seconds < self.max_age_seconds and
                    instance.usage_count < self.max_requests_per_browser):
                    
                    instance.last_used = datetime.now()
                    instance.usage_count += 1
                    self.in_use.append(instance)
                    self.stats['pool_hits'] += 1
                    
                    # Rotate fingerprint periodically
                    if instance.usage_count % 5 == 0:
                        instance.fingerprint = AdvancedFingerprint.generate_fingerprint()
                        self.stats['fingerprints_rotated'] += 1
                    
                    return instance
                else:
                    # Close unhealthy/old browser
                    await self._close_browser(instance)
            
            # Pool miss - create new browser
            self.stats['pool_misses'] += 1
            
            # Check if we're at max capacity
            if len(self.in_use) >= self.max_size:
                logger.warning("Browser pool at maximum capacity")
                # Wait for a browser to be returned
                while len(self.in_use) >= self.max_size:
                    await asyncio.sleep(0.1)
                    
            # Create new instance
            instance = await self._create_browser_instance()
            self.in_use.append(instance)
            return instance
    
    @asynccontextmanager
    async def acquire_browser(self):
        """
        Acquire a browser with CDP bypass and stealth measures applied
        Returns: (browser, context, page) tuple
        """
        start_time = time.time()
        instance = None
        context = None
        page = None
        
        try:
            # Get browser instance
            instance = await self._get_browser_instance()
            
            # Create stealth context with fingerprint
            context = await StealthIntegration.create_stealth_context(
                instance.browser,
                proxy_config=None,  # Can be passed if needed
                fingerprint=instance.fingerprint
            )
            
            # Track context
            instance.contexts.append(context)
            
            # Create page
            page = await context.new_page()
            
            # Apply CDP bypass if not already protected
            if not instance.cdp_protected:
                await self.cdp_bypass.apply_cdp_bypass(page)
                instance.cdp_protected = True
                self.stats['cdp_protections_applied'] += 1
            
            # Apply additional stealth measures
            await StealthIntegration.apply_page_stealth(page)
            
            # Update acquisition stats
            acquisition_time = time.time() - start_time
            self._update_acquisition_stats(acquisition_time, hit=instance.usage_count > 1)
            
            logger.debug(f"🎯 Browser acquired in {acquisition_time:.2f}s (CDP protected, fingerprint applied)")
            
            yield instance.browser, context, page
            
        except Exception as e:
            logger.error(f"Error acquiring browser: {e}")
            raise
            
        finally:
            # Cleanup
            if page:
                try:
                    await page.close()
                except Exception:
                    pass
            
            if context:
                try:
                    await context.close()
                    if instance and context in instance.contexts:
                        instance.contexts.remove(context)
                except Exception:
                    pass
            
            if instance:
                await self._return_browser(instance)
    
    async def _return_browser(self, instance: BrowserInstance) -> None:
        """Return browser to pool"""
        
        async with self.lock:
            if instance in self.in_use:
                self.in_use.remove(instance)
            
            # Check if browser should be recycled
            if (instance.is_healthy and 
                instance.age_seconds < self.max_age_seconds and
                instance.usage_count < self.max_requests_per_browser and
                not self.shutdown):
                
                # Return to pool
                self.pool.append(instance)
                self.stats['browsers_recycled'] += 1
                logger.debug(f"Browser returned to pool (usage: {instance.usage_count})")
            else:
                # Close browser
                await self._close_browser(instance)
    
    async def _close_browser(self, instance: BrowserInstance) -> None:
        """Close browser and cleanup"""
        
        try:
            # Close all contexts
            for context in instance.contexts[:]:
                try:
                    await context.close()
                except Exception:
                    pass
            
            # Close browser
            await instance.browser.close()
            
            # Stop playwright
            await instance.playwright.stop()
            
            logger.debug("Browser instance closed")
            
        except Exception as e:
            logger.error(f"Error closing browser: {e}")
    
    async def _is_browser_healthy(self, instance: BrowserInstance) -> bool:
        """Check if browser instance is healthy"""
        
        try:
            # Create a test context
            test_context = await instance.browser.new_context()
            test_page = await test_context.new_page()
            
            # Try to navigate
            response = await test_page.goto('about:blank', timeout=5000)
            
            # Cleanup
            await test_page.close()
            await test_context.close()
            
            return response is not None
            
        except Exception:
            return False
    
    async def _ensure_minimum_pool_size(self) -> None:
        """Ensure pool has minimum number of browsers"""
        
        async with self.lock:
            current_size = len(self.pool) + len(self.in_use)
            
            while current_size < self.min_size and not self.shutdown:
                try:
                    instance = await self._create_browser_instance()
                    self.pool.append(instance)
                    current_size += 1
                except Exception as e:
                    logger.error(f"Failed to create browser: {e}")
                    break
    
    async def _health_check_loop(self) -> None:
        """Background task for health checking"""
        
        while not self.shutdown:
            try:
                await asyncio.sleep(self.health_check_interval)
                await self._perform_health_checks()
            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.error(f"Health check error: {e}")
    
    async def _perform_health_checks(self) -> None:
        """Perform health checks on idle browsers"""
        
        async with self.lock:
            for instance in self.pool[:]:
                if not await self._is_browser_healthy(instance):
                    instance.health_check_failures += 1
                    
                    if instance.health_check_failures > 3:
                        logger.warning(f"🏥 Removing unhealthy browser from pool")
                        self.pool.remove(instance)
                        await self._close_browser(instance)
                else:
                    instance.health_check_failures = 0
            
            self.stats['health_checks'] += 1
    
    async def _maintenance_loop(self) -> None:
        """Background task for pool maintenance"""
        
        while not self.shutdown:
            try:
                await asyncio.sleep(30)  # Run every 30 seconds
                await self._perform_maintenance()
            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.error(f"Maintenance error: {e}")
    
    async def _perform_maintenance(self) -> None:
        """Perform pool maintenance tasks"""
        
        # Remove old browsers
        async with self.lock:
            for instance in self.pool[:]:
                if instance.age_seconds > self.max_age_seconds:
                    logger.info(f"🧹 Removing old browser (age={instance.age_seconds:.0f}s)")
                    self.pool.remove(instance)
                    await self._close_browser(instance)
                elif instance.idle_seconds > self.max_idle_seconds:
                    logger.info(f"🧹 Removing idle browser (idle={instance.idle_seconds:.0f}s)")
                    self.pool.remove(instance)
                    await self._close_browser(instance)
        
        # Ensure minimum pool size
        await self._ensure_minimum_pool_size()
        
        # Log stats periodically
        if self.stats['total_acquisitions'] % 100 == 0 and self.stats['total_acquisitions'] > 0:
            self._log_stats()
    
    def _update_acquisition_stats(self, acquisition_time: float, hit: bool) -> None:
        """Update acquisition statistics"""
        
        self.stats['total_acquisitions'] += 1
        
        # Update moving average
        prev_avg = self.stats['avg_acquisition_time']
        n = self.stats['total_acquisitions']
        self.stats['avg_acquisition_time'] = (prev_avg * (n - 1) + acquisition_time) / n
    
    def _log_stats(self) -> None:
        """Log pool statistics"""
        
        hit_rate = (self.stats['pool_hits'] / max(1, self.stats['total_acquisitions'])) * 100
        
        logger.info(f"""
📊 Enhanced Browser Pool Statistics:
  - Pool size: {len(self.pool)} available, {len(self.in_use)} in use
  - Browsers created: {self.stats['browsers_created']}
  - Browsers recycled: {self.stats['browsers_recycled']}
  - Pool hit rate: {hit_rate:.1f}%
  - CDP protections: {self.stats['cdp_protections_applied']}
  - Fingerprints rotated: {self.stats['fingerprints_rotated']}
  - Avg acquisition time: {self.stats['avg_acquisition_time']:.3f}s
  - Health checks: {self.stats['health_checks']}
  - Memory usage: {psutil.Process().memory_info().rss / 1024 / 1024:.1f} MB
        """)
    
    async def shutdown_pool(self) -> None:
        """Gracefully shutdown the pool"""
        
        logger.info("🛑 Shutting down enhanced browser pool...")
        self.shutdown = True
        
        # Cancel background tasks
        if self._health_check_task:
            self._health_check_task.cancel()
        if self._maintenance_task:
            self._maintenance_task.cancel()
        
        # Close all browsers
        async with self.lock:
            all_browsers = self.pool + self.in_use
            for instance in all_browsers:
                await self._close_browser(instance)
            
            self.pool.clear()
            self.in_use.clear()
        
        logger.info("✅ Browser pool shutdown complete")
    
    def get_stats(self) -> Dict[str, Any]:
        """Get current pool statistics"""
        
        return {
            **self.stats,
            'pool_size': len(self.pool),
            'in_use': len(self.in_use),
            'hit_rate': (self.stats['pool_hits'] / max(1, self.stats['total_acquisitions'])) * 100
        }


# Global browser pool instance
_browser_pool: Optional[EnhancedBrowserPool] = None


async def get_browser_pool(config: Optional[Dict[str, Any]] = None) -> EnhancedBrowserPool:
    """Get or create the global browser pool instance"""
    global _browser_pool
    
    if _browser_pool is None:
        _browser_pool = EnhancedBrowserPool(config)
        await _browser_pool.initialize()
    
    return _browser_pool


async def shutdown_browser_pool() -> None:
    """Shutdown the global browser pool"""
    global _browser_pool
    
    if _browser_pool:
        await _browser_pool.shutdown_pool()
        _browser_pool = None


# For backward compatibility
BrowserPool = EnhancedBrowserPool