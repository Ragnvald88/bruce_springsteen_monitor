# src/models/browser.py
"""
Browser profile and configuration models
"""
...
class BrowserProfile:
    ...
    # Profile metadata
    quality: ProfileQuality = ProfileQuality.TIER_3
    ...
    platform_stats: Dict[str, ProfileMetrics] = field(default_factory=dict)
    
    # Optimization settings
    data_optimization_level: DataOptimizationLevel = DataOptimizationLevel.BALANCED
    block_resources: Set[str] = field(default_factory=set)
    ...
    def __post_init__(self):
        ...
        # Convert dict proxy config to ProxyConfig object
        if self.proxy_config and isinstance(self.proxy_config, dict):
            ...
            self.proxy_config = ProxyConfig(
                ...
                proxy_type=protocol,
                country_code=self.proxy_config.get('country_code'),
                sticky_session=self.proxy_config.get('sticky_session', True),
                proxy_provider=self.proxy_config.get('proxy_provider')
            )
        ...
        for platform in Platform:
            if platform.value not in self.platform_stats:
                self.platform_stats[platform.value] = ProfileMetrics()
    ...
    def should_rotate_proxy(self) -> bool:
        ...
        recent_detections = sum(
            len([e for e in stats.detection_events 
    ...
    def record_usage(self, success: bool, response_time_ms: float = 0,
        ...
        if platform and platform in self.platform_stats:
            ...
            if captcha_encountered:
                ...
                if success:
                    stats.solved_captchas += 1
                stats.captcha_solve_rate = (
                    stats.solved_captchas / stats.total_captchas
                    if stats.total_captchas > 0 else 0.0
                )
    ...
    def get_success_rate(self, platform: Optional[str] = None) -> float:
        ...
        if platform:
            ...
            attempts = sum(s.attempts for s in self.platform_stats.values())
            successes = sum(s.successes for s in self.platform_stats.values())
